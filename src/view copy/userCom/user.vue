    ...
    <div class="tabs_two_box" v-for="(item,len1) in tabdata3" :key="len1">
      <div class="tabs_info">
        ...
        <div :class="item.is_act?'tabs_two act':'tabs_two'">
          ...
          <div
            v-if="item.saleNo<=5 &&searchValue3!='实时完成率' "
            class="tabs_normal_cont tabs_lc tabs_lmoney"
          >{{searchValue3=="累计完成"?item.finishMoney:searchValue3=="标准销售额"?item.standardFinishMoney:''}}</div>
          <div
            v-if="item.saleNo>5&&searchValue3!='实时完成率' &&tabdata3xskword.length==tabdata3.length"
            class="tabs_normal_cont tabs_lc tabs_lmoney"
          >
            <span v-if="item.saleNo<=tabdata3.length-5">
              <span v-if="(searchValue3=='累计完成' &&item.finishMoney<=0)||(searchValue3=='标准销售额'&&item.standardFinishMoney<=0)">{{searchValue3=="累计完成"?item.finishMoney:searchValue3=="标准销售额"?item.standardFinishMoney:''}}</span>
              <span v-else>***</span>
            </span>
            <span v-else>{{searchValue3=="累计完成"?item.finishMoney:searchValue3=="标准销售额"?item.standardFinishMoney:''}}</span>
          </div>
           <div
            v-if="item.saleNo>5&&searchValue3!='实时完成率' &&tabdata3xskword.length!=tabdata3.length"
            class="tabs_normal_cont tabs_lc tabs_lmoney"
          ...
          <!-- <div v-if="item.saleNo>tabdata3.length-5&&searchValue3!='实时完成率'  &&tabdata3xskword.length==tabdata3.length" class="tabs_normal_cont tabs_lc tabs_lmoney">{{searchValue3=="累计完成"?item.finishMoney:searchValue3=="实时完成率"?item.finishRate.split('/')[0]:searchValue3=="标准销售额"?item.standardFinishMoney:''}}</div> -->
          <!-- <div v-if="item.saleNo>5&&item.searchValue3!='实时完成率'  &&tabdata3xskword.length!=tabdata3.length" class="tabs_normal_cont tabs_lc tabs_lmoney">***</div> -->
          <div
...
export default {
  props: ["tabdata3", "pagenum", "searchValue3", "tabdata3xskword"],
  data() {
    return {
