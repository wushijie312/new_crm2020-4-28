<template>
  <div style="font-size:14px;padding-bottom:53px;" class="all_change_box">
    <div class="qu_body">
        ...
        <div class="border_b">
          <div v-if="item.select_type=='radio'">
            ...
            <span style="display:block;">
              <el-button
                ...
                @click="item.select_link=='el-icon-view'?bfjlHandle():item.select_link=='el-icon-connection'?khgxHandle(item.select_link):item.select_link=='el-icon-search'?zpxqHandle(item,index):item.select_link=='el-icon-set-up'?khgxHandle(item.select_link):item.select_link=='el-icon-check'?khgxHandle(item.select_link):''"
          ...
          <div v-if="item.select_type=='input'">
            ...
            <div
              v-for="(itemchild,lenchild) in item.select_values"
              :key="'input1-'+lenchild"
              style="font-size:15px;color:#606266;"
              class="m_t10"
            >
              {{itemchild.name}}
              <el-input
          ...
          <div v-if="item.select_type=='no'">
            <p class="qu_border_tit">{{item.sort}}、{{item.question}}</p>
            <span style="display:block;">
              <span v-if="item.select_link &&item.select_link=='el-icon-thumb'">
                <span v-for="(itemurl,lenurl) in item.select_url" :key="lenurl">
                  <el-button
                    size="mini"
                    type="primary"
                    :icon="itemurl.link!='el-icon-folder-checked'?item.select_link:itemurl.link"
                    @click="itemurl.link!='el-icon-folder-checked'?ggxzHandle(itemurl):ggxzjlHandle(item)"
                    style="margin-right:10px;"
                  >{{itemurl.name}}</el-button>
                </span>
              </span>

              <el-button
                v-if="item.select_link &&item.select_link!='el-icon-thumb'"
                size="mini"
                type="primary"
                :icon="item.select_link"
                @click="item.select_link=='el-icon-view'?bfjlHandle():item.select_link=='el-icon-connection'?khgxHandle(item.select_link):item.select_link=='el-icon-search'?zpxqHandle(item,index):item.select_link=='el-icon-set-up'?yjjlHandle(item.select_link):item.select_link=='el-icon-check'?xzryHandle(item.select_link):''"
              >{{item.select_link_name}}</el-button>
            </span>
          </div>
          <el-input
      ...
      <div  class="qu_submit_btn_box">
        <el-button
         :type="disabled || activeName=='高管支持'?'primary':''"
        class="qu_submit_btn"
          @click="cancelbackHandel"
        >返回</el-button>
        <el-button
        v-if="!disabled && activeName!='高管支持'"
        class="qu_submit_btn"
          type="primary"
          @click="submitHandel"
        >提交</el-button>
    ...
    <el-drawer
      ...
      style="font-size:12px;"
    >
      <div class="qu_bfcard" v-if="ggxzjlData!=0">
        <div class="qu_bfcard_box" v-for="(item,index) in ggxzjlData" :key="index">
          <p class="qu_bfcard_btime">
            <span class="qu_bfcard_round"></span>
            {{item.create_time?item.create_time:'尚未拜访'}}
          </p>
          <div class="qu_card_cont_bf">
            <div class="clearfix qu_ggxzjl_a">
              <p class="qu_bfcard_b">
                <span>协助类型：</span>
                <span
                  class="qu_bfcard_bdec"
                >{{item.tagcode==1?'帮助':item.tagcode==2?'建议':item.tagcode==3?'任务':''}}</span>
              </p>
              <p class="qu_bfcard_b">
                <span>协助时间：</span>
                <span class="qu_bfcard_bdec">
            ...
            <p class="qu_bfcard_b">
              <span>协助内容：</span>
              <span class="qu_ggxzjl_b qu_bfcard_bdec">{{item.msginfo}}</span>
            </p>
            <p class="qu_bfcard_b m_b0">
              <span>协助人员：</span>
              <span class="qu_bfcard_bdec">{{item.create_name}}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="qu_card" v-if="khgxData!=0">
        ...
        >
          ...
          <div v-if="!item.is_show" class="qu_card_b clearfix">
            <p class="clearfix">
              <i
                :class="item.phone?'el-icon-phone-outline fl qu_card_b_icon':item.contact_person_email?'el-icon-message fl qu_card_b_icon':item.birthday?'el-icon-date fl qu_card_b_icon':item.remark?'el-icon-tickets fl qu_card_b_icon_bz':''"
              ></i>
          ...
          <div v-if="item.is_show" class="qu_card_b clearfix">
            <p v-if="item.phone" class="clearfix">
              <i class="el-icon-phone-outline fl qu_card_b_icon"></i>
              {{item.phone}}
            </p>
    ...
    >
      <div class="qu_card" v-if="khgxData!=0">
        <div
          :class="item.is_edit?'qu_card_cont qu_card_active':'qu_card_cont'"
          v-for="(item,index) in khgxData"
          :key="index"
        >
        ...
        >
          <div class="qu_card_icon">
            ...
            <el-checkbox
              ...
              @change="editHandelSave(item,index)"
              v-model="item.is_contact_person"
              :disabled="disabled"
            ></el-checkbox>
          </div>
          <div class="qu_card_a clearfix">
...
export default {
  ...
  props: ["customerId", "tagSource", "customerName", "activeName"],
  watch: {
  ...
  methods: {
    ...
    submitHandel() {
      var qu_lists = this.qu_lists;
      for (var i = 0; i < qu_lists.length; i++) {
        var lists = qu_lists[i];
        if (!lists.q_id) {
          this.$message.error({
            message: `${lists.sort}、${lists.question}未填写，请填写完成提交!`
          });
          return;
        }
      }
    ...
    ggxzjlHandle(item) {
      var userId = item.select_url[0].link.slice(
        item.select_url[0].link.indexOf("=") + 1
      );
      this.stop();
      this.CHANGE_OVERFLOW_HIDE(true);
      this.ggxzjldraw = true;
      //  // getkehu
      wqtaskinfo({ customerId: this.customerId, userId }).then(res => {
        this.ggxzjlData = res.data;
        console.log(this.ggxzjlData);
      });
    },
    ggxzHandle(item) {
...
<style scoped>
.qu_submit_btn{
  width:100%;display:block;padding: 11px 15px;font-weight:normal;font-size:15px;
}
.qu_submit_btn_box {
  position: fixed;
  max-width: 850px;
...
@media screen and (min-width: 850px) {
  .qu_ggxzjl_a .qu_bfcard_b:nth-child(1) {
    float: left;
  }
  .qu_ggxzjl_a .qu_bfcard_b:nth-child(2) {
    float: right;
    margin-right: 108px;
  }
  .qu_ggxzjl_b.qu_bfcard_bdec {
    margin-right: 108px;
  }
}
</style>