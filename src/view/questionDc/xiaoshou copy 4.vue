      ...
      <div class="qu_bfcard" v-if="ggxzjlData!=0">
        <div class="qu_bfcard_box" v-for="(item,index) in ggxzjlData" :key="index">
          ...
          <div class="qu_card_cont_bf">
            ...
            <p class="qu_bfcard_b">
              <span>协助时间：</span>
              <span class="qu_bfcard_bdec">{{item.create_time}}<span style="display:inline-block;padding:0 5px;">至</span>{{item.end_time}} 00:00:00</span>
            </p>

            <p class="qu_bfcard_b">
              <span>协助内容：</span>
              <span class="qu_bfcard_bdec">{{item.msginfo}}</span>
            </p>
            <p class="qu_bfcard_b">
              <span>协助人员：</span>
      ...
      <div class="qu_card" v-if="khgxData!=0">
        <div class="qu_card_cont" v-for="(item,index) in khgxData" :key="index">
          ...
          <div class="qu_card_b clearfix">
            <p>
              <i
                :class="item.phone?'el-icon-phone-outline qu_card_b_icon':item.contact_person_email?'el-icon-message qu_card_b_icon':item.birthday?'el-icon-date qu_card_b_icon':item.remark?'el-icon-tickets qu_card_b_icon':''"
              ></i>
              {{item.phone?item.phone:item.contact_person_email?item.contact_person_email:item.birthday?item.birthday:item.remark?item.remark:''}}
            </p>
          </div>
  ...
  methods: {
    ...
    submitHandel() {
      ...
      for(var i=0;i<qu_lists.length;i++){
        ...
        if(!lists.q_id){
          this.$message.error({ message: `您有未填写的选项，请填写完成提交` });
          return;
        }
      }
