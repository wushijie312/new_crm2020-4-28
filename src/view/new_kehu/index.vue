<template>
  <div class="wrapper wrapwhite">
    ...
    <div class="content" ref="wrapper">
      ...
      >
        ...
        >
          ...
          <span style="float:right;font-size:0.3rem;font-weight:900;" @click="$router.go(-1)">返回首页</span>
        </p>
      </h3>
      <!--搜索条件-->
      <div class="searchBox">
      ...
      <div class="customerType">
        <span v-for="(citem,ckey) in customerType.options" :key="ckey">
          <el-button
            type="text"
            :class="{'choosed':customerType.checked==citem.source}"
            @click="getcustomerType(citem.source)"
          >{{citem.result}}</el-button>
          ...
          <div class="dataitem">
            ...
            <p @click="foldFromParent('item'+key)" class="c_s_info" style="margin-bottom: 4px;">
              <span v-show="item.flowInFlag!='true'">
                {{item.dept_name}}-{{item.leader_name}}
                <span>销售：{{item.seller}}</span>
              </span>
              <span v-show="item.flowInFlag=='true'">
                <el-button
                  slot="reference"
                  type="text"
                  @click="flowin(item,$event)"
                  style="textDecoration: underline;font-size: 0.3rem;padding: 0px;"
                  size="mini"
                >转入客户</el-button>
              </span>
            </p>
            ...
            >
              <div
                @click="wjdcHandel(item)"
                style="line-height:0.8rem;background:#21aefb;margin:0 auto;color: #FFFFFF;border-radius: 4px;text-align: center;"
              >
                <span style="position:relative;">
                  调查问卷
                  <img
                    :src="newPic"
                    style="position:absolute;top: 4px;right: -46px;width: 24px;"
                    class="wjdc_kehu_icon"
                  />
                </span>
              </div>
            </div>
            ...
            >
              <div
                @click="foldFromParent('itemsaleInfos'+key)"
                style="line-height:0.8rem;background:#21aefb;margin:0 auto;color: #FFFFFF;border-radius: 4px;text-align: center;"
              >
                销售数据
                <Fold
                  :ref="'itemsaleInfos'+key"
                  changekey="saleInfoshide"
                  :index="key"
                  :parantData="tabdata2"
                  @foldChange="reciveChange"
                ></Fold>
              </div>
            ...
            >
              <div
                @click="foldFromParent('itemvisits'+key)"
                style="line-height:0.8rem;background:#21aefb;margin:0 auto;color: #FFFFFF;border-radius: 4px;text-align: center;"
              >
                最新拜访进展
                <Fold
                  :ref="'itemvisits'+key"
                  changekey="visitshide"
                  :index="key"
                  :parantData="tabdata2"
                  @foldChange="reciveChange"
                ></Fold>
              ...
              <Visit
                v-show="item.visitshide"
                :supplierId="item.supplier_id"
                :companyName="item.company_name"
                :visitsdataSon.sync="item.visits"
              ></Visit>
            </div>
            <!--派单-->
            <div
            ...
            >
              <div
                @click="foldFromParent('itemtasks'+key)"
                style="line-height:0.8rem;background:#21aefb;margin:0 auto;color: #FFFFFF;border-radius: 4px;text-align: center;"
              >
                最新派单情况
                <Fold
              ...
              <Order
                v-show="item.taskshide"
                :supplierId="item.supplier_id"
                :companyName="item.company_name"
                :orderdataSon.sync="item.tasks"
              ></Order>
            </div>
            <!--客户关系-->
            <div
            ...
            >
              <div
                @click="foldFromParent('itemrelations'+key)"
                style="line-height:0.8rem;background:#21aefb;margin:0 auto;color: #FFFFFF;border-radius: 4px;text-align: center;"
              >
                关系图谱
                <Fold
                  :ref="'itemrelations'+key"
                  changekey="relationshide"
                  :index="key"
                  :parantData="tabdata2"
                  @foldChange="reciveChange"
                ></Fold>
    ...
    <el-dialog title :visible.sync="dialogCon.dialogVisible" width="80%" center>
      <span>
        确定转入客户
        <span style="color:#409EFF ;">{{dialogCon.val.company_name}}</span>？
      </span>
...
export default {
  ...
  methods: {
    ...
    getCustomerTag(callback) {
      //获取客户类型
      customerTag({ userid: localStorage.getItem("userid") }).then(res => {
        if (res.code == 200 && res.dataList) {
          this.customerType.options = this.customerType.options.concat(
            res.dataList
          );
          if (this.customerType.options.length)
            this.customerType.checked = this.customerType.options[0].source;
...
.text.item > div:nth-child(4n + 3),
.text.item > div:nth-child(4n + 4) {
  background: #f0f9eb;
}
...
.empty {
  ...
  text-align: center;
  color: #999999;
  background: url(../../assets/img/normal/empty.png) no-repeat center;
