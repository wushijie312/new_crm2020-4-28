      	...
      	<el-drawer
		      ...
		      <div style="width:98%;margin-left:1%;padding: 0px 10px;">
			      ...
			      <el-form-item label="省市">
			      	<el-cascader ref="cascaderAddr" placeholder="请选择省市" :options="options" clearable v-model="searchData.selected" @change="handleChange" :props="{value:'label'}"  style="width: 100%;display: inline-block;"></el-cascader>
			      </el-form-item>
        ...
        <el-card class="box-card">
          ...
          >
            ...
            <el-table-column prop="position" label="付款方式" class-name="selfColumn" width='90'>
              <template v-slot="scope">
                {{scope.row.resultSectionByStr?scope.row.resultSectionByStr:'/'}}
              </template>
            </el-table-column>
            <el-table-column prop="correlation_person" label="接单金额(万元)" class-name="selfColumn" width='140'>
              <template v-slot="scope">
                {{scope.row.orderPriceStr?scope.row.orderPriceStr:'/'}}
              </template>
            </el-table-column>
            <el-table-column prop="relation" label="接单数量" class-name="selfColumn" width='90'>
              ...
              <template v-slot="scope">
                {{scope.row.cityName==scope.row.provinceName?scope.row.areaName:scope.row.cityName}}
              </template>
            </el-table-column>
...
//  	return 'text-align: center;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;display: table-cell!important;'
    ...
    getdata() {
      ...
      providerList({
        ...
        provinceName:this.searchData.selected?this.searchData.selected[0]:"",
        cityName:this.searchData.selected?this.searchData.selected[1]:"",
        orderMoney:this.searchData.orderMoney,
        orderCount:this.searchData.orderCount?this.searchData.orderCount:null,
      }).then(res => {
        if(res.code==200){
        	if (this.pagenum == 1) {
