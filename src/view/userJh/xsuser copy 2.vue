<template>
 ...
 <div class="menu-head-top50"></div>
    ...
    <div class="create_edit">
      <i class="el-icon-circle-plus" fontSize="50px" ></i>
    </div> 
    <div v-show="!drop" class="create_daohang" @click="changeDrop">
      <span class="create_daohang_a">
        <i class="el-icon-s-promotion create_daohang_a_icon"></i>快速导航
      </span>
    </div>
    <div class="create_ks_fixed" v-show="drop" @click="dropcancel">
      <div class="create_ks_box">
        <div
          v-for="(item,index) in dropList"
          :key="'ks'+index"
          class="create_ks_bg"
          @click="goCreate(item.url)"
        >
          <p class="create_ks_list clearfix">
        ...
        <i class="el-icon-circle-close create_ks_close"></i>
      </div>
    </div>
    <div
      icon="el-icon-edit"
      style="width:50px;height:50px;position:fixed;right:10px;top:40%;"
      @click="goCreate('/xsCreateData')"
    >
      <i class="el-icon-circle-plus" fontSize="50px" ></i>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      tabdata3: [],
      drop:false,
      dropList:[{name:'销售日报创建',url:'/xsCreateData'},{name:'拜访日报创建',url:'/createData'},{name:'BP&周报创建',url:'/rbcreate'},{name:'我的派单创建',url:'/create'},{name:'客户池创建',url:'/addCustomer'},]
    };
  },
  created() {},
  mounted() {
